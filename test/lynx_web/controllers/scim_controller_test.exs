...
defmodule LynxWeb.SCIMControllerTest do
  ...
  @scim_token "test-scim-bearer-token"
  ...
  describe "SCIM authentication" do
    ...
    test "returns 404 when SCIM is disabled", %{conn: conn} do
      Application.put_env(:lynx, :scim_enabled, false)
  ...
  describe "SCIM discovery" do
    test "GET /ServiceProviderConfig", %{conn: conn} do
      conn =
        conn
        |> scim_conn()
    ...
    test "GET /ResourceTypes", %{conn: conn} do
      conn =
        conn
        |> scim_conn()
  ...
  describe "SCIM Users" do
    test "POST /Users creates a user", %{conn: conn} do
      body = %{
    ...
    test "GET /Users lists users", %{conn: conn} do
      # Create a user first
      conn
      |> scim_conn()
      |> post("/scim/v2/Users", %{
    ...
    test "GET /Users with filter", %{conn: conn} do
      conn
      |> scim_conn()
      |> post("/scim/v2/Users", %{
    ...
    test "GET /Users/:id returns a user", %{conn: conn} do
      create_conn =
        conn
        |> scim_conn()
        |> post("/scim/v2/Users", %{
    ...
    test "GET /Users/:id returns 404 for missing user", %{conn: conn} do
      conn =
        conn
        |> scim_conn()
    ...
    test "PATCH /Users/:id deactivates a user", %{conn: conn} do
      create_conn =
        conn
        |> scim_conn()
        |> post("/scim/v2/Users", %{
    ...
    test "DELETE /Users/:id deactivates a user", %{conn: conn} do
      create_conn =
        conn
        |> scim_conn()
        |> post("/scim/v2/Users", %{
  ...
  describe "SCIM Groups" do
    test "POST /Groups creates a group", %{conn: conn} do
      conn =
        conn
        |> scim_conn()
        |> post("/scim/v2/Groups", %{
    ...
    test "GET /Groups lists groups", %{conn: conn} do
      conn
      |> scim_conn()
      |> post("/scim/v2/Groups", %{
    ...
    test "DELETE /Groups/:id deletes a group", %{conn: conn} do
      create_conn =
        conn
        |> scim_conn()
        |> post("/scim/v2/Groups", %{
